  ...
  describe "#perform" do
    ...
    context "campaign is beginning" do
      ...
      it "should return recipients that have not yet begun" do
        expect(Campaigns::StepUpRecipientJob).not_to receive(:perform_later).with(recipient1.id, "lancement")
        expect(Campaigns::StepUpRecipientJob).to receive(:perform_later).with(recipient2.id, "lancement")
        expect(Campaigns::StepUpRecipientJob).to receive(:perform_later).with(recipient5.id, "lancement")
        expect(Campaigns::StepUpRecipientJob).to receive(:perform_later)
          .with(recipient9.id, "lancement") # started do receive mails
        expect(Campaigns::StepUpRecipientJob).not_to receive(:perform_later)
    ...
    context "campaign is in relance2" do
      ...
      it "should return recipients in relance1" do
        expect(Campaigns::StepUpRecipientJob).not_to receive(:perform_later).with(recipient1.id, "relance2")
        expect(Campaigns::StepUpRecipientJob).to receive(:perform_later).with(recipient4.id, "relance2")
        expect(Campaigns::StepUpRecipientJob).to receive(:perform_later).with(recipient6.id, "relance2")
        Campaigns::RunCampaignJob.new.perform(campaign.id)
      end
    end
    ...
    context "campaign is ending" do
      ...
      it "should return recipients that are in relance3" do
        expect(Campaigns::StepUpRecipientJob).not_to receive(:perform_later).with(recipient1.id, "fin")
        expect(Campaigns::StepUpRecipientJob).to receive(:perform_later).with(recipient7.id, "fin")
        Campaigns::RunCampaignJob.new.perform(campaign.id)
      end
    end
  end
