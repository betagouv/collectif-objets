...
feature "Galerie objet photos palissy", type: :feature, js: true do
  context "three photos" do
    ...
    it "should be navigable" do
      ...
      expect(page).to have_css("img[src*='objet1.jpg']")
      expect(find("h1", text: /La Sainte-Famille/)).to be_visible
      click_on "Voir les 3 photos"
      expect(page).to have_css(".co-galerie")
      expect(page).to have_css('button[disabled][title="photo précédente"]')
      click_on "photo suivante"
      click_on "photo suivante"
      expect(page).to have_css('button[disabled][title="photo suivante"]')
      click_on "photo précédente"
      click_on "Fermer"
      expect(page).to have_no_css(".co-galerie")
    end
  end
end
