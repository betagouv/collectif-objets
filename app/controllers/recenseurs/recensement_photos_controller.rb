...
module Communes
  class RecensementPhotosController < BaseController
    ...
    def create
      return render "recenseurs/recensements/edit" unless @wizard.update(permitted_params)

      respond_to do |format|
        format.html { redirect_to_edit }
        format.turbo_stream do
          url = recenseurs_commune_objet_recensement_photos_path(@recensement.commune, @recensement.objet, @recensement)
          render turbo_stream: [
            turbo_stream.append(
            ...
            turbo_stream.replace("upload_form", PhotoUploadComponent.new(url:).render_in(view_context))
          ]
        end
      end
    end
    ...
    def redirect_to_edit
      redirect_to(edit_recenseurs_commune_objet_recensement_path(@recensement.commune,
                                                                 @recensement.objet,
                                                                 @recensement,
